...
# 问题改写Prompt
REWRITE_PROMPT = """你是一个C++面试问题的语义分析专家。请对用户的面试问题进行分析：

1. 提取核心知识点关键词（3-5个技术术语）
2. 改写为更利于检索的标准查询

用户问题：{question}

必须以纯JSON格式输出，不要添加markdown代码块标记，不要添加解释文字，直接输出JSON，例如：
{{"keywords": ["关键词1", "关键词2"], "rewritten_query": "改写后的标准查询"}}"""
